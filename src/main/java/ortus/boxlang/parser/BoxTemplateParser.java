...
public class BoxTemplateParser extends AbstractParser {
	...
	private BoxTryCatch toAst( File file, CatchBlockContext node ) {
		BoxExpr			exception	= new BoxIdentifier( "bxcatch", null, null );
	...
	private BoxExpr toAst( File file, BoxTemplateGrammar.QuotedStringContext node ) {
	...
	private BoxIfElse toAst( File file, BoxTemplateGrammar.IfContext node ) {
		...
		BoxExpr				condition	= parseBoxExpression( node.ifCondition.getText(), getPosition( node.ifCondition ) );
		...
		for ( int i = node.elseIfCondition.size() - 1; i >= 0; i-- ) {
			...
			BoxExpr		thisCondition	= parseBoxExpression( node.elseIfCondition.get( i ).getText(), getPosition( node.elseIfCondition.get( i ) ) );
	...
	private BoxStatement toAst( File file, SetContext set ) {
		...
		return new BoxExpression( parseBoxExpression( set.expression().getText(), getPosition( set.expression() ) ), getPosition( set ), getSourceText( set ) );
	}
	...
	public BoxExpr parseBoxExpression( String code, Position position ) {
		try {
