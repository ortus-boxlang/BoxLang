...
public class PendingQuery {
	...
	public ExecutedQuery execute(Connection conn) {
		try (PreparedStatement statement = conn.prepareStatement(this.sql, PreparedStatement.RETURN_GENERATED_KEYS) ) {
			// The param index starts from 1
			for (int i = 1; i <= this.parameters.size(); i++) {
				QueryParameter param = this.parameters.get(i - 1);
				statement.setObject(i, param.getValue(), param.getSqlTypeAsInt(), param.getScaleOrLength());
			}
