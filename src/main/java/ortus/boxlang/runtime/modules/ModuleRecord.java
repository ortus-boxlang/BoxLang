...
public class ModuleRecord {
	...
	/**
	 * The global services provided by the module. (Any {@link IService} implementation)
	 */
	private List<IService>							globalServices				= new ArrayList<>();
	 ...
	 */
	private Map<Key, DriverShim>					JDBCDriverShims				= new HashMap<>();

	/**
	 * The JDBC drivers registered by this module. (Any {@link IJDBCDriver} implementation)
	 */
	private List<IJDBCDriver>						JDBCDrivers					= new ArrayList<>();

	/**
	 * The schedulers provided by the module. (Any {@link IScheduler} implementation)
	 */
	private List<IScheduler>						schedulers					= new ArrayList<>();

	/**
	 * The cache providers registered by this module. (Any {@link ICacheProvider} implementation)
	 */
	private List<Class<? extends ICacheProvider>>	cacheProviders				= new ArrayList<>();
	/**
	 * The Java interceptors provided by the module. (Any {@link IInterceptor} implementation)
	 */
	private List<IInterceptor>						javaInterceptors			= new ArrayList<>();
	...
	public ModuleRecord register( IBoxContext context ) {
		...
		ServiceLoader.load( IService.class, this.classLoader )
		    ...
		    .forEach( service -> {
			    ...
			    this.runtime.putGlobalService( service.getName(), service );
		    } );
		...
		ServiceLoader.load( Driver.class, this.classLoader )
		    ...
		    .forEach( driver -> {
			    ...
			    this.JDBCDriverShims.put( Key.of( driver.getClass().getName() ), driverShim );
			    try {
		...
		ServiceLoader.load( IJDBCDriver.class, this.classLoader )
		    ...
		    .map( ServiceLoader.Provider::get )
		    .forEach( driver -> {
			    this.JDBCDrivers.add( driver );
			    this.runtime.getDataSourceService().registerDriver( driver );
		    } );
		...
		ServiceLoader.load( IScheduler.class, this.classLoader )
		    ...
		    .forEach( scheduler -> {
			    ...
			    this.runtime.getSchedulerService()
		...
		ServiceLoader.load( ICacheProvider.class, this.classLoader )
		    ...
		    .map( ServiceLoader.Provider::type )
		    .forEach( provider -> {
			    this.cacheProviders.add( provider );
			    this.runtime.getCacheService().registerProvider( Key.of( provider.getSimpleName() ),
			        provider );
		    } );
		...
		ServiceLoader.load( IInterceptor.class, this.classLoader )
		    ...
		    .map( ServiceLoader.Provider::get )
		    .forEach( targetInterceptor -> {
			    this.javaInterceptors.add( targetInterceptor );
			    interceptorService.register( targetInterceptor, this.settings );
		    } );
	...
	public ModuleRecord unregister( IBoxContext context ) {
		...
		this.JDBCDriverShims.forEach( ( key, driver ) -> {
			try {
		...
		this.javaInterceptors.forEach( interceptor -> interceptorService.unregister( interceptor ) );
		this.javaInterceptors.clear();
