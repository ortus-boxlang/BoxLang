...
public class ApplicationClassListener extends ApplicationListener {
	...
	public ApplicationClassListener( IClassRunnable listener, RequestBoxContext context ) {
		...
		// If there is no application name, make one up.
		if ( !this.settings.containsKey( Key._NAME ) ) {
			this.settings.put( Key._NAME, "Autogenerated_Application_Name_"
			    + ( String ) BoxRuntime.getInstance().getFunctionService().getGlobalFunction( Key.of( "createGUID" ) ).invoke( context, false ) );
		}
	}
