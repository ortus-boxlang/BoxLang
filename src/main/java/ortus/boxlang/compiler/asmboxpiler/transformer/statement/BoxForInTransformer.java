...
public class BoxForInTransformer extends AbstractTransformer {
	...
	public List<AbstractInsnNode> transform( BoxNode node, TransformerContext context ) {
		...
		int startStackHeight = tracker.getCurrentStackHeight();
		...
		nodes.addAll( transpiler.transform( forIn.getBody(), context ) );

		nodes.addAll( tracker.popStackEntries( tracker.getCurrentStackHeight() - startStackHeight + 1 ) );
	...
	private List<AbstractInsnNode> assignVar( BoxForIn forIn, int iteratorIndex, TransformerContext context ) {
		...
		return new BoxAssignmentTransformer( ( AsmTranspiler ) transpiler ).transformEquals(
		    ...
		    modifiers,
		    ( forIn.getHasVar() ? "var " : "" ) + forIn.getVariable().getSourceText() );
	}
