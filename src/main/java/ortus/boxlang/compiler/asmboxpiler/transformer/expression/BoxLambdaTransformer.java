...
/**
 * Transform a Lambda using the static method + method handle pattern.
 * Instead of generating a separate inner class, this generates:
 * 1. A static invoker method on the enclosing class
 * 2. An instantiation of Lambda with a method reference to the static invoker
 */
public class BoxLambdaTransformer extends AbstractTransformer {
	...
	public List<AbstractInsnNode> transform( BoxNode node, TransformerContext context, ReturnValueContext returnContext ) throws IllegalStateException {
		...
		// Reserve a slot in the lambdaInstantiations list to prevent nested lambdas from getting the same index
		int			lambdaIndex	= transpiler.getLambdaInstantiations().size();
		transpiler.getLambdaInstantiations().add( null );
		...
		// Generate the Lambda instantiation bytecode: new Lambda(name, arguments, returnType, access, annotations, documentation, modifiers, defaultOutput,
		// imports, sourceType, path, ClassName::invokerMethodName)
		List<AbstractInsnNode> instantiation = new ArrayList<>();

		// NEW Lambda
		instantiation.add( new TypeInsnNode( Opcodes.NEW, Type.getInternalName( Lambda.class ) ) );
		instantiation.add( new InsnNode( Opcodes.DUP ) );

		// Arg 1: name (Key) - Lambda.defaultName
		...
		instantiation.addAll( transpiler.transformAnnotations( boxLambda.getAnnotations() ) );
		...
		// INVOKESPECIAL Lambda.<init>(Key, Argument[], String, Access, IStruct, IStruct, List, boolean, List, BoxSourceType, ResolvedFilePath, Function)V
		...
		// Store the Lambda instantiation bytecode in the reserved slot
		transpiler.getLambdaInstantiations().set( lambdaIndex, instantiation );

		// Return: lambdas.get(lambdaIndex) - retrieves the pre-built Lambda from the static field
		List<AbstractInsnNode> result = new ArrayList<>();
		result.add( new FieldInsnNode( Opcodes.GETSTATIC,
		    declaringType.getInternalName(),
		    "lambdas",
		    Type.getDescriptor( List.class ) ) );
		result.add( new LdcInsnNode( lambdaIndex ) );
		result.add( new MethodInsnNode( Opcodes.INVOKEINTERFACE,
		    Type.getInternalName( List.class ),
		    "get",
		    Type.getMethodDescriptor( Type.getType( Object.class ), Type.INT_TYPE ),
		    true ) );
		result.add( new TypeInsnNode( Opcodes.CHECKCAST, Type.getInternalName( Lambda.class ) ) );
