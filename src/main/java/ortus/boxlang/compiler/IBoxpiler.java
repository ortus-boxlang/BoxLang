...
public interface IBoxpiler {
	 ...
	 * </p>
	 */
	public static final int		BYTECODE_VERSION	= 11;
	...
	/**
	 * The prefix used for generated function invoker method names.
	 *
	 * <p>
	 * Function bodies are compiled into static methods named with this prefix followed by an
	 * underscore and the (sanitized) function name. For example, a function named {@code runRemote}
	 * produces a method called {@code invokeFunction_runRemote}.
	 * </p>
	 */
	static final String INVOKE_FUNCTION_PREFIX = "invokeFunction_";

	/**
	 * Sanitizes a string so it is safe to use as part of a Java method or identifier name.
	 *
	 * <p>
	 * BoxLang and CFML allow characters in function names (such as hyphens) that are not valid
	 * in Java identifiers. This method replaces any such character with a unique hex-encoded
	 * representation ({@code $XXXX}) to avoid illegal identifier errors while preserving
	 * uniqueness (no two different inputs produce the same output). The {@code $} character
	 * itself is encoded as {@code $$} to prevent ambiguity with escape sequences.
	 * </p>
	 *
	 * <p>
	 * Characters that pass {@link Character#isJavaIdentifierPart(char)} (except {@code $}) are
	 * kept as-is. This means normal ASCII letters, digits, and underscores pass through unchanged.
	 * </p>
	 *
	 * @param name The raw function/identifier name from BoxLang or CFML source code
	 *
	 * @return A string safe for use as a Java identifier, with illegal characters hex-encoded
	 *
	 * @see #unsanitizeFromJavaIdentifier(String)
	 */
	static String sanitizeForJavaIdentifier( String name ) {
		// Fast path: check if any sanitization is actually needed
		...
		for ( int i = 0; i < name.length(); i++ ) {
			char c = name.charAt( i );
			if ( c == '$' ) {
				// Escape literal $ as $$ to avoid ambiguity with hex escape sequences
				sb.append( "$$" );
			} else if ( !Character.isJavaIdentifierPart( c ) ) {
				// Encode invalid char as $ + 4-digit uppercase hex
	...
	/**
	 * Reverses the sanitization performed by {@link #sanitizeForJavaIdentifier(String)},
	 * restoring the original function/identifier name.
	 *
	 * <p>
	 * This is used primarily in {@code ExceptionUtil} to recover the original BoxLang function
	 * name from a compiled method name for display in stack traces and error messages.
	 * </p>
	 *
	 * @param sanitized The sanitized identifier string (as produced by {@link #sanitizeForJavaIdentifier})
	 *
	 * @return The original unsanitized name
	 *
	 * @see #sanitizeForJavaIdentifier(String)
	 */
	static String unsanitizeFromJavaIdentifier( String sanitized ) {
		// Fast path: if there's no $, nothing was sanitized
		...
		for ( int i = 0; i < sanitized.length(); i++ ) {
			...
			if ( c == '$' && i + 1 < sanitized.length() ) {
				char next = sanitized.charAt( i + 1 );
				if ( next == '$' ) {
					// $$ → literal $
					sb.append( '$' );
					i++;
				} else if ( i + 4 < sanitized.length() ) {
					// $XXXX → decoded character
					String hex = sanitized.substring( i + 1, i + 5 );
					try {
						sb.append( ( char ) Integer.parseInt( hex, 16 ) );
						i += 4;
					} catch ( NumberFormatException e ) {
						// Not a valid escape, keep the $ as-is
						sb.append( c );
					}
				} else {
					sb.append( c );
