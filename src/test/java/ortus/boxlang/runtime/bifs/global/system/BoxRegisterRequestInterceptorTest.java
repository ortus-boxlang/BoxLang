...
@Disabled
public class BoxRegisterRequestInterceptorTest {
	...
	void testItCanRegisterAClass() {
		instance.executeSource(
		    """
		    	result = boxRegisterRequestInterceptor( new src.test.bx.Interceptor() )
		    """,
		    context );
	...
	void testItCanRegisterADynamicObject() {
		...
		instance.executeSource(
		    """
		    	result = boxRegisterRequestInterceptor( variables.dynamicInterceptor );
		    """,
		    context );
	...
	void testItCanRegisterAClosureOrLambda() {
		instance.executeSource(
		    """
		      	result = boxRegisterRequestInterceptor( () => { return true; }, "afterCacheElementRemoved" );
		    	result2 = boxRegisterRequestInterceptor( () -> { return true; }, "afterCacheElementRemoved" );
		    """,
		    context );
	...
	void testItCanThrowAnExceptionUsingAClosureOrLambdaWithNoStates() {
		...
		assertThrows( BoxRuntimeException.class, () -> {
			instance.executeSource(
			    """
			      	result = boxRegisterRequestInterceptor( () => { return true; } );
			    """,
			    context );
		} );
	}
