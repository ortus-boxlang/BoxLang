...
public class ArrayFindAllNoCaseTest {
    ...
    @DisplayName( "It should find strings in a case insensitive manner" )
    @Test
    ...
    public void testMatchStructs() {
        instance.executeSource(
            """
                nums = [ 1, [3], { test: true } ];
                result = nums.findAllNoCase( { test: true } );
            """,
            context );
        Array found = ( Array ) variables.dereference( result, false );
        assertThat( found.size() ).isEqualTo( 1 );
    ...
    public void testMatchNumberVariationsBoolean() {
        instance.executeSource(
            """
                nums = [ 1, "1", 1.0, "1.0", true, "true", "yes" ];
                result = nums.findAllNoCase( true );
            """,
            context );
        Array found = ( Array ) variables.dereference( result, false );
        assertThat( found.size() ).isEqualTo( 7 );
    ...
    public void testMatchUsingUDF() {
        instance.executeSource(
            """
                ...
                result = nums.findAllNoCase( value -> value == "red" );
            """,
            context );
        Array found = ( Array ) variables.dereference( result, false );
        assertThat( found.size() ).isEqualTo( 2 );
