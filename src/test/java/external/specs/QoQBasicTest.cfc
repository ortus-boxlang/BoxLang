		...
		describe( 'QofQ' , function(){
			...
			describe( 'Query grouping' , function(){
				...
				it( 'Aggregate select with no group by against empty query returns 1 row of empty strings' , function() {
					...
					expect( actual.avg[1] ).toBeNull();
					expect( actual.min[1] ).toBeNull();
					expect( actual.max[1] ).toBeNull();
				...
				it( 'Aggregates ignore null values' , function() {
					...
					expect( actual.sum2[1] ).toBeNull();
					expect( actual.avg2[1] ).toBeNull();
					expect( actual.min2[1] ).toBeNull();
					expect( actual.max2[1] ).toBeNull();
				});
